...
SELECT DISTINCT a.immed_prev_stop, a.dest_route_type, b.immed_prev_route_type, b.dest_mysqlid, b.total_time, a.origin_mysqlid
FROM 
(
  SELECT a1.immed_prev_stop, a1.origin_mysqlid, a2.dest_route_type
  FROM 
  (
    SELECT dest_mysqlid, origin_mysqlid, immed_prev_stop, mode_route_mysqlid
    FROM mbta_data.mbta_map_weekday_72000
    WHERE dest_mysqlid = 8545
    AND origin_mysqlid = 8523
  ) AS a1
  LEFT JOIN
  (
