-- # Copyright 2013 by Douglas R. Tillberg <doug@transitboston.com>.  This file is part of the mbta_map project. 
-- # It is subject to the license terms in the LICENSE file found in the top-level directory of this distribution 
-- # and at https://github.com/doug0/mbta_map/blob/master/LICENSE. No part of the mbta_map project, including this file, 
-- # may be copied, modified, propagated, or distributed except according to the terms contained in the LICENSE file.

INSERT INTO mbta_data.sched_mapping_stops 
(
  mysql_id, 
  stop_id, 
  stop_code, 
  stop_name, 
  stop_desc, 
  stop_lat, 
  stop_lon, 
  stop_geo,
  location_type, 
  parent_station, 
  timestamp, 
  feed_id_first, 
  feed_id_last, 
  data_hash
)
SELECT mysql_id, stop_id, stop_code, stop_name, stop_desc, stop_lat, stop_lon,
    POINT(stop_lat, stop_lon), 
    location_type, parent_station, timestamp, feed_id_first, feed_id_last, data_hash
FROM mbta_data.sched_stops;