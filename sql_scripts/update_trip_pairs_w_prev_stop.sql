...
SELECT SQL_BIG_RESULT z.origin_mysqlid AS origin_mysqlid, z.dest_mysqlid as dest_mysqlid, AVG(TIME_TO_SEC(z.travel_time_secs)) AS avg_travel_time_secs, STD(TIME_TO_SEC(z.travel_time_secs)) AS std_dev_travel_time_secs, COUNT(z.origin_mysqlid) AS trip_count
FROM
(
  SELECT a.origin_mysqlid AS origin_mysqlid, b.dest_mysqlid AS dest_mysqlid, a.trip_mysqlid AS trip_mysql, b.stop_sequence AS dest_sequence, TIMEDIFF(b.arrival_time, a.departure_time) AS travel_time_secs
  FROM
  (
    SELECT a1.origin_mysqlid AS origin_mysqlid, a1.departure_time AS departure_time, a1.trip_id AS trip_id, a1.stop_sequence AS stop_sequence, a2.mysql_id AS trip_mysqlid
    FROM
    (
      SELECT a6.origin_mysqlid AS origin_mysqlid, a5.departure_time AS departure_time, a5.trip_id AS trip_id, a5.stop_sequence AS stop_sequence
      FROM
      (
  ...
  LEFT JOIN
  (
    SELECT b1.dest_mysqlid AS dest_mysqlid, b1.arrival_time AS arrival_time, b2.trip_id AS trip_id, b1.stop_sequence AS stop_sequence, b2.mysql_id AS trip_mysqlid
    FROM
    (
